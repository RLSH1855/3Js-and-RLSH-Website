...
function modelMatches(row, garage) {
  ...
  const legacy = cmo === gmo || cmo.indexOf(gmo) !== -1 || gmo.indexOf(cmo) !== -1 || cmo.indexOf('SILVERADO/SIERRA') !== -1 && (gmo.indexOf('SILVERADO') !== -1 || gmo.indexOf('SIERRA') !== -1) || cmo === 'CANYON/COLORADO' && (gmo === 'CANYON' || gmo === 'COLORADO') || cmo === '1500/2500/3500' && (gmo.indexOf('RAM') !== -1 || gmo.indexOf('1500') !== -1 || gmo.indexOf('2500') !== -1);
...
function distinctBedSizes(entries) {
  ...
  return Object.keys(byClass).sort((a, b) => BED_CLASS_ORDER[a] - BED_CLASS_ORDER[b]).map(cls => {
    const counts = byClass[cls].counts;
    const label = Object.keys(counts).sort((a, b) => counts[b] - counts[a])[0];
    return {
      cls,
      label,
      bedIn: byClass[cls].bedIn
    };
  });
}
