/**
 * 3J's Auto Body — Quote API Worker
 *
 * POST /api/quote
...
async function createOrderWithServices(customerId, vehicleId, data, env) {
  ...
  // Add primary product as service line item
  if (data.product) {
  ...
  // Add each add-on as a separate service line item
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // 3. Create order with service line items
      ...
      // Shopmonkey failed — fall back to Google Sheets
      console.error('Shopmonkey error, falling back to Sheets:', smError.message);

      try {
        var sheetsResult = await fallbackToSheets(data, env);
        return jsonResponse({
          ok: true,
          status: 'ok',
          source: 'sheets',
